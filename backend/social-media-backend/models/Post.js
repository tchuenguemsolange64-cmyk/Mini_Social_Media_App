const { supabase, supabaseAdmin } = require('./index');
...
class Post {
  ...
  // Get user's posts
  static async getUserPosts(userId, limit = 20, offset = 0, currentUserId = null) {
    let query = supabase
      .from('posts')
      .select(`
        *,
  ...
  static async getExplore(limit = 20, offset = 0, currentUserId = null) {
    const { data, error } = await supabase
      .from('posts')
      .select(`
        *,
  ...
  // Get post likes
  static async getLikes(postId, limit = 20, offset = 0) {
    const { data, error } = await supabase
      .from('likes')
      .select(`
        user:user_id (
  ...
  // Bookmark a post
  static async bookmark(postId, userId) {
    const { data, error } = await supabase
      .from('bookmarks')
      .insert({ post_id: postId, user_id: userId })
      .select()
  ...
  // Remove bookmark
  static async removeBookmark(postId, userId) {
    const { error } = await supabase
      .from('bookmarks')
      .delete()
      .eq('post_id', postId)
  ...
  static async getBookmarks(userId, limit = 20, offset = 0) {
    const { data, error } = await supabase
      .from('bookmarks')
      .select(`
        post:post_id (
  ...
  // Share/Repost a post
  static async share(postId, userId, comment = null) {
    const { data, error } = await supabase
      .from('shares')
      .insert({
        post_id: postId,
